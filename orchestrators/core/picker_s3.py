...
def _get_s3_and_cfg() -> Tuple[any, str, str, str]:
    ...
    npcs_key = os.getenv("PICKER_NPC_KEY", "picker/npcs.json")
    
    s3 = boto3.client("s3", region_name=region, config=Config(retries={"max_attempts": 5, "mode": "standard"}))
    return s3, bucket, trending_posts_prefix, articles_key, dialogues_key, posts_key, story_histories_key, current_key, history_prefix, npcs_key

def _iso_utc_now() -> str:
    return time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())

def _read_articles_array() -> Optional[List[Dict[str, str]]]:
    s3, bucket, _, articles_key, _, _, _, _, _, _ = _get_s3_and_cfg()
...
def _read_dialogues_dict() -> Dict[str, List[List[Dict[str, Any]]]]:
    s3, bucket, _, _, dialogues_key, _, _, _, _, _ = _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"
    try:
...
def _read_npcs_array() -> Optional[List[str]]:
    s3, bucket, _, _, _, _, _, _, _, npcs_key = _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"
    try:
        obj = s3.get_object(Bucket=bucket, Key=npcs_key)
        data = json.loads(obj["Body"].read().decode("utf-8"))
    except s3.exceptions.NoSuchKey:
        return None
    except Exception as e:
        print(f"[s3] read_npcs error: {e.__class__.__name__}: {e}")
        return None

    if not isinstance(data, list):
        print(f"[json] expected top-level array at s3://{bucket}/{npcs_key}")
        return None
...
def _read_posts_array() -> Optional[List[int]]:
    s3, bucket, _, _, _, posts_key, _, _, _, _ = _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"
    try:
...
def _read_story_histories_dict() -> Dict[str, Dict[str, List[str]]]:
    s3, bucket, _, _, _, _, story_histories_key, _, _, _  = _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"
    try:
...
def get_random_npc() -> Optional[str]:
    npcs = _read_npcs_array()
    if not npcs:
        return None
    index = random.randrange(len(npcs))
    npc = npcs[index]
    return npc
...
def read_current() -> Optional[Dict[str, Any]]:
    s3, bucket, _, _, _, _, _, current_key, _, _= _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"
    try:
...
def write_current_and_history(persona_id: str, post: Dict[str, Any], reply_goal: int):
    s3, bucket, _, _, _, _, _, current_key, history_prefix, _ = _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"
    ...
    print(f"[s3] wrote current and history -> s3://{bucket}/{current_key} (history={hist_key})")
...
def write_dialogue(persona_id: str, post: Dict[str, Any]):
    ...
    print(f"[s3] wrote dialogue -> s3://{bucket}/{dialogues_key}")

def write_npc(username: str):
    s3, bucket, _, _, _, _, _, _, _, npcs_key = _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"

    data = _read_npcs_array()
    if not isinstance(data, list):
        data = []
    u = (username or "").strip()
    if not u:
    ...
    s3.put_object(Bucket=bucket, Key=npcs_key, Body=body, ContentType="application/json", CacheControl="no-cache")
    
    print(f"[s3] wrote npc -> s3://{bucket}/{npcs_key}")
...
def write_story_history(persona_id: str, story_phase: str, text: str):
    ...
    print(f"[s3] wrote story_history -> s3://{bucket}/{story_histories_key}")

def write_trending_posts(trending_posts: List[Dict[str, Any]]):
    s3, bucket, trending_posts_prefix, _, _, _, _, _, _, _ = _get_s3_and_cfg()
    assert bucket, "AWS_S3_BUCKET is required"
